...
final class ProfileInfoTableViewCell: UITableViewCell {
  ...
  internal var showPickerViewController: (() -> Void)?
  
  // MARK: - Private Properties
  private let changePhotoButton: UIButton = {
    let changePhoto = UIButton()
    changePhoto.setTitle("Change photo", for: .normal)
    changePhoto.titleLabel?.font = .systemFont(ofSize: 8)
  ...
  private func setupUI() {
    ...
    fullNameLabel.text = "\(UserDefaultsService.userModel.firstName)" + " " + "\(UserDefaultsService.userModel.lastName)"
